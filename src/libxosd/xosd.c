	  ...
	  int percent, int is_slider, int set_color)
{
  ...
  if (osd->shadow_offset)
      ...
      for (xx = x, i = 0; i < nbars; xx += barw, i++)
	{
	  struct bar *b = &(bar[is_slider ? (i == on) : (i < on)]);
	  XFillRectangle (osd->display, d, gc,
			  xx + osd->shadow_offset, b->y + osd->shadow_offset,
			  b->w, b->h);
	}
    }
  /* Bar/Slider */
	...
	case Expose:
	  ...
	  if (report.xexpose.count == 0)
	      ...
	      for (line = 0; line < osd->number_lines; line++)
		{
	      ...
	      XShapeCombineMask (osd->display, osd->window, ShapeBounding, 0,
				 0, osd->mask_bitmap, ShapeSet);
	      XFlush (osd->display);
...
static int
force_redraw (xosd * osd, int line)	/* Requires mutex lock. */
{
  assert (osd);
  resize (osd);
  for (line = 0; line < osd->number_lines; line++)
...
static int
set_colour (xosd * osd, const char *colour)	/* Requires mutex lock. */
{
  int retval = 0;
...
xosd_set_shadow_colour (xosd * osd, const char *colour)
{
  ...
  pthread_mutex_lock (&osd->mutex);
  retval =
    parse_colour (osd, &osd->shadow_colour, &osd->shadow_pixel, colour);
  force_redraw (osd, -1);
  pthread_mutex_unlock (&osd->mutex);
...
static void
update_pos (xosd * osd)		/* Requires mutex lock. */
{
  assert (osd);
  switch (osd->pos)
    {
...
static void
set_timeout (xosd * osd)	/* Requires mutex lock. */
{
  assert (osd);
...
/** Hide current lines. **/
static void
hide (xosd * osd)		/* Requires mutex lock. */
{
  assert (osd);
  osd->mapped = 0;
...
static void
show (xosd * osd)		/* Requires mutex lock. */
{
  assert (osd);
  osd->mapped = 1;
